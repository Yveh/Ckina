...
private:
	...
	*/
	...
	void _search(node &p, sjtu::vector<key_t> &vect1,sjtu::vector<value_t> &vect2, const key_t & key, std::function<bool(const key_t &, const key_t &)> compar) {
		...
		if (p.type) {
			...
			while (!compar(key, *nthk_b(b, x)) ) {
				if (!compar(*nthk_b(b, x), key)) vect1.push_back(*nthk_b(b,x)),vect2.push_back(*nthv_b(b,x));
				++x;
				if (x == pp.sz) {
		...
		return _search(cn, vect1,vect2, key, compar);
	}
...
public:	
	...
	void search(const key_t & key, std::function<bool(const key_t &, const key_t &)> compar,
				sjtu::vector<key_t> &vect1,sjtu::vector<value_t> &vect2) {
		if (empty()) return;
		node r = read_node(root);                     // read_node(root) is not a lvalue 
		_search(r, vect1,vect2, key, compar);
	}
